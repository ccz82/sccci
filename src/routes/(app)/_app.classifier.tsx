...
const mediaSchema = z.object({
  ...
  classification: z.enum(["casual", "diplomatic"]),
...
function RouteComponent() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      for (const item of files) {
        ...
        formData.append("classification", parsed.classification);
        formData.append("analysis", parsed.analysis || getAnalysis(parsed.classification));
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const droppedFiles = Array.from(e.dataTransfer.files).map(file => ({
      ...
      type: "event" as const,
      image: file,
      classification: "diplomatic" as const,
      analysis: getAnalysis("diplomatic"),
    }));
  ...
  const updateFile = (index: number, updates: Partial<MediaItem>) => {
    setFiles(prev => {
      const newFiles = [...prev]
      // If classification is updated, update analysis too
      if (updates.classification) {
        newFiles[index] = {
          ...newFiles[index],
          ...updates,
          analysis: getAnalysis(updates.classification as 'casual' | 'diplomatic'),
        };
      } else {
                ...
                <div key={i} className="bg-background border border-border rounded-md flex items-start justify-between gap-4 p-4">
                  <div className='flex flex-row gap-4'>
                    ...
                    <div className='flex flex-col gap-2'>
                      ...
                      <Select value={item.classification} onValueChange={val => updateFile(i, { classification: val as 'casual' | 'diplomatic' })}>
                      ...
                      <Label>Caption</Label>
                      <Input value={item.analysis || ''} readOnly />
                    </div>
                  </div>
                  <Button
                    variant="outline"
