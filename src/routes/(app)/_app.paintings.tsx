...
export const Route = createFileRoute('/(app)/_app/paintings')({
  loader: async () => {
    // Load albums with type "painting"
    const paintingAlbums = await pb.collection("albums").getFullList({
      filter: 'type = "painting"'
    });
    
    // Load individual media items from painting albums
    const allMedia = await pb.collection("media").getFullList();
    const paintingMedia = allMedia.filter(media => {
      const album = paintingAlbums.find(album => album.id === media.album);
      return album !== undefined;
    });
    
    return { 
      albums: paintingAlbums, 
      media: paintingMedia,
          ...
          <ScrollArea className="h-1 grow rounded-md border">
            {filteredMedia.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 text-center text-muted-foreground">
                <ImageIcon size={50} />
                <p className="mt-4">No painting images found.</p>
              </div>
            ) : (
              <div className={cn(
                "p-4",
        ...
        <TabsContent value="albums" className="flex flex-col gap-4">
          <ScrollArea className="h-1 grow rounded-md border">
            {filteredAlbums.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 text-center text-muted-foreground">
                <FolderIcon size={50} />
                <p className="mt-4">No painting albums found.</p>
              </div>
            ) : (
              <div className={cn(
                "p-4",
